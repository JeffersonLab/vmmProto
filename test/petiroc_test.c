...
int
process_buf(int *buf, int nwords, FILE * f, int print)
{
  static int tag = 15;
...
//  petiroc_adc_init(1);
  ...
  while(1)
	...
	{
#if EVENT_DATA_SAVE
	  fwrite(event_buffer, event_buffer_len, sizeof(event_buffer[0]),
		 f_events);
#endif
	  nwords_current += event_buffer_len;

	  nevents_current +=
	    process_buf(event_buffer, event_buffer_len, NULL,
			EVENT_DATA_PRINT);
	}
      else
	usleep(1000);
      ...
      diff =
	t_cur.tv_sec + 1.0E-6 * t_cur.tv_usec - t_last.tv_sec -
	1.0E-6 * t_last.tv_usec;
      if(diff > 1.0)
	{
#if EVENT_DATA_SAVE
	  fflush(f_events);
